...
#define MEMOIZE_AND_RET(_s,_p, cond) { auto ret = (cond); dp[_s][_p] = ret; return ret;}

#define CHECK_IF_MEMOIZED(_s,_p) { auto it =  dp.find(s);   \
        if ( it != dp.end() ) {                             \
